...
public:
  ...
  public:
    ...
    virtual uint16_t next_sequence_number(datagram_handle hdl) = 0;

    /// Adds a message with a future sequence number to the pending messages
    /// of a given endpoint context.
    virtual void add_pending(uint16_t seq, basp::endpoint_context& ep,
  ...
  bool handle(execution_unit* ctx, new_datagram_msg& dm, basp::endpoint_context& ep);
  ...
  bool handle(execution_unit* ctx, const Handle& hdl, header& hdr,
              ...
              optional<basp::endpoint_context&> ep, optional<uint16_t> port) {
