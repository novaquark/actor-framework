...
uint16_t basp_broker_state::next_sequence_number(datagram_handle hdl) {
  auto i = ctx_udp.find(hdl);
...
void basp_broker_state::add_pending(uint16_t seq, basp::endpoint_context& ep,
                                    basp::header hdr,
...
behavior basp_broker::make_behavior() {
  ...
  return {
    ...
    [=](pending_atom, datagram_handle hdl, uint16_t seq) {
      auto& ep = state.ctx_udp[hdl];
      auto itr = ep.pending.find(seq);
      if (itr != ep.pending.end()) {
        // drop missing packets if they did not arrive in the meantime
        ep.seq_incoming = seq;
